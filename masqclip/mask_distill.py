...
class MaskDistill(nn.Module):
    ...
    def NMS(self, scores, masks):
        ...
        for mask, sample_mask in zip(masks, sample_masks):
            ...
            if max_dice < self.dice_threshold:
                new_masks.append(mask)
                new_sample_masks.append(sample_mask)
...
class MaskFormer(nn.Module):
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            for mask_cls_result, mask_pred_result, input_per_image, image_size in zip(
                ...
                height = input_per_image["image"].shape[1]
                width = input_per_image["image"].shape[2]
    ...
    def prepare_targets(self, targets, images):
        ...
        for targets_per_image in targets:
            ...
            new_targets.append(
                {
                    "labels": torch.zeros_like(targets_per_image.gt_classes),  # class-agnostic
                    "masks": padded_masks,
                }
            )
        return new_targets
